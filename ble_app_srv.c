...
uint32_t ble_app_service_init(ble_app_service_t * p_app_service, \
    ...
    err_code = app_custom_char_add(p_app_service, &(p_app_service->acc_X) , \
                               READ_ON,WRITE_OFF,NOTIFY_ON, APP_BLE_ACC_X, &init_16value, 2);
    VERIFY_SUCCESS(err_code);     
           
    err_code = app_custom_char_add(p_app_service, &(p_app_service->acc_Y) , \
                               READ_ON,WRITE_OFF,NOTIFY_ON, APP_BLE_ACC_Y, &init_16value, 2);
    VERIFY_SUCCESS(err_code);            
    
    err_code = app_custom_char_add(p_app_service, &(p_app_service->acc_Z) , \
                               READ_ON,WRITE_OFF,NOTIFY_ON, APP_BLE_ACC_Z, &init_16value, 2);
    VERIFY_SUCCESS(err_code);            
    
    err_code = app_custom_char_add(p_app_service, &(p_app_service->gyro_X) , \
                               READ_ON,WRITE_OFF,NOTIFY_ON, APP_BLE_GYR_X, &init_16value, 2);
    VERIFY_SUCCESS(err_code);            
    
    err_code = app_custom_char_add(p_app_service, &(p_app_service->gyro_Y) , \
                               READ_ON,WRITE_OFF,NOTIFY_ON, APP_BLE_GYR_Y, &init_16value, 2);
    VERIFY_SUCCESS(err_code);            
    
    err_code = app_custom_char_add(p_app_service, &(p_app_service->gyro_Z) , \
                               READ_ON,WRITE_OFF,NOTIFY_ON, APP_BLE_GYR_Z, &init_16value, 2);
    VERIFY_SUCCESS(err_code);            
...
//    NRF_LOG_INFO("Updating characteristic! err_code: %x",err_code); 
    ...
    if ((conn_handle != BLE_CONN_HANDLE_INVALID)) 
    {
        ...
        NRF_LOG_INFO("Notifiy user about updated"); 
    }
    else
    {
